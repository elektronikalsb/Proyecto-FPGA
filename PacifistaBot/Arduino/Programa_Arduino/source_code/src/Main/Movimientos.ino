...
void posicion_agachado() {
  ...
  posicionesObjetivo[0] = 80;//28 tobillo derecho
  posicionesObjetivo[1] = 60;//20 pierna derecha
  posicionesObjetivo[2] = 80;//110 tobillo izquierdo
  posicionesObjetivo[3] = 100;//63 pierna izquierda
  posicionesObjetivo[4] = 70;//95 cadera derecha
  posicionesObjetivo[5] = 75;//82 cadera izquierda
  posicionesObjetivo[6] = 68;//64 pie izquierdo
  posicionesObjetivo[7] = 68;//58 pie derecho
...
void posicion_hombros() {
  ...
  case MODO_HOMBROS_ADELANTE:
    posicionesEstandarBrazoIzquierdo[POS_SERVO_HOMBRO] = 140;//125
    posicionesEstandarBrazoDerecho[POS_SERVO_HOMBRO] = 0;//43
    break;
  ...
  for (int servo = 0; servo < numServos; servo++) {
    ...
    ajustesHombros[servo][PERIODO] = 1000;//2500 oreka ez galtzeko
...
void movimiento_puno_izquierdo() {
  ...
  ajustesPunoIzquierdo[0][AMPLITUD] = 72;
...
void movimiento_puno_derecho() {
  ...
  ajustesPunoDerecho[0][AMPLITUD] = 72;
...
void extender_brazos() {
  const byte numServos = 2;
  short ajustesHombros[numServos][5];
  byte servosHombrosExtender[numServos][2];
  short posicionesObjetivo[numServos];

  servosHombrosExtender[0][TIPO_SERVO] = TIPO_SERVO_BRAZO_IZQUIERDO;
  servosHombrosExtender[0][POS_SERVO] = POS_SERVO_BRAZO;

  servosHombrosExtender[1][TIPO_SERVO] = TIPO_SERVO_BRAZO_DERECHO;
  servosHombrosExtender[1][POS_SERVO] = POS_SERVO_BRAZO;

  switch (MODO_BRAZOS_EXT) {
  case MODO_BRAZOS_EXTENDIDO:
    posicionesEstandarBrazoIzquierdo[POS_SERVO_BRAZO] = 37;
    posicionesEstandarBrazoDerecho[POS_SERVO_BRAZO] = 82;
    break;
  case MODO_BRAZOS_PLEGADO:
    posicionesEstandarBrazoIzquierdo[POS_SERVO_BRAZO] = 97;
    posicionesEstandarBrazoDerecho[POS_SERVO_BRAZO] = 22;
    break;
  }

  posicionesObjetivo[0] = posicionesEstandarBrazoIzquierdo[POS_SERVO_BRAZO];
  posicionesObjetivo[1] = posicionesEstandarBrazoDerecho[POS_SERVO_BRAZO];

  for (int servo = 0; servo < numServos; servo++) {
    int pos_actual = get_posicion_actual(servosHombrosExtender[servo][TIPO_SERVO], servosHombrosExtender[servo][POS_SERVO]);
    int desfase = pos_actual - posicionesObjetivo[servo];
  ...
  ejecutar_movimiento(servosHombrosExtender, ajustesHombros, numServos, 0.25f);
}

void invincible_mod(int repeticiones){
  const byte numServos = 1;
